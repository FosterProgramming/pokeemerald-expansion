...
bool32 IsDamageMoveUnusable(u32 battlerAtk, u32 battlerDef, u32 move, u32 moveType)
{
    ...
    case EFFECT_POLTERGEIST:
        if (AI_DATA->items[battlerDef] == ITEM_NONE || gFieldStatuses & STATUS_FIELD_MAGIC_ROOM || AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_KLUTZ))
            return TRUE;
        break;
        ...
        for (i = 0; i < gMovesInfo[move].numAdditionalEffects; i++)
        {
            switch (gMovesInfo[move].additionalEffects[i].moveEffect)
            {
                ...
                case MOVE_EFFECT_DEF_SPDEF_DOWN:
                    if ((gMovesInfo[move].additionalEffects[i].self && !AI_BATTLER_HAS_TRAIT(battlerAtk, ABILITY_CONTRARY))
                        || (noOfHitsToKo != 1 && AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_CONTRARY) && !DoesBattlerIgnoreAbilityChecks(abilityAtk, move)))
                        return TRUE;
                    break;
...
bool32 DoesBattlerIgnoreAbilityChecks(u32 atkAbility, u32 move)
{
    ...
    if (HasMoldBreakerTypeAbility(sBattler_AI) || gMovesInfo[move].ignoresTargetAbility)
        return TRUE;
...
bool32 ShouldTryOHKO(u32 battlerAtk, u32 battlerDef, u32 atkAbility, u32 defAbility, u32 move)
{
    ...
    else if (BattlerHeldItemHasEffect(battlerDef, HOLD_EFFECT_FOCUS_SASH, TRUE) && AtMaxHp(battlerDef))
        return FALSE;

    if (!DoesBattlerIgnoreAbilityChecks(atkAbility, move) && AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_STURDY))
        return FALSE;
            ...
            if (!CanAIFaintTarget(battlerAtk, battlerDef, 0)) // Can't KO foe otherwise
            {
                ...
                if (CanAIFaintTarget(battlerAtk, battlerDef, 2))
                {
                    ...
                    if (!IS_MOVE_STATUS(move) && ((AI_DATA->shouldSwitch & (1u << battlerAtk))
                        || (AtMaxHp(battlerDef) && (BattlerHeldItemHasEffect(battlerDef, HOLD_EFFECT_FOCUS_SASH, TRUE)
                ...
                {
                    if (!IS_MOVE_STATUS(move) && (AtMaxHp(battlerDef) && (BattlerHeldItemHasEffect(battlerDef, HOLD_EFFECT_FOCUS_SASH, TRUE)
                        || (B_STURDY >= GEN_5 && AISearchTraits(AIBattlerTraits, ABILITY_STURDY))
                        || AISearchTraits(AIBattlerTraits, ABILITY_MULTISCALE)
                        || AISearchTraits(AIBattlerTraits, ABILITY_SHADOW_SHIELD))))
                        return SHOULD_PIVOT;   // pivot to break sash/sturdy/multiscale
            ...
            else // Foe can 3HKO+ AI
            {
                ...
                else if (CanAIFaintTarget(battlerAtk, battlerDef, 2))
                {
                    ...
                    if (IS_MOVE_STATUS(move) && ((AI_DATA->shouldSwitch & (1u << battlerAtk)) //Damaging move
                     ...
                     || (BattlerHeldItemHasEffect(battlerDef, HOLD_EFFECT_FOCUS_SASH, TRUE) && AtMaxHp(battlerDef))))
...
bool32 AI_CanPoison(u32 battlerAtk, u32 battlerDef, u32 defAbility, u32 move, u32 partnerMove)
{
    if (!CanBePoisoned(battlerAtk, battlerDef, GetBattlerAbility(battlerDef))
      ...
      || PartnerMoveEffectIsStatusSameTarget(BATTLE_PARTNER(battlerAtk), battlerDef, partnerMove))
        return FALSE;
    else if (!AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_CORROSION) && (IS_BATTLER_OF_TYPE(battlerDef, TYPE_POISON) || IS_BATTLER_OF_TYPE(battlerDef, TYPE_STEEL)))
        return FALSE;
    else if (IsValidDoubleBattle(battlerAtk) && AI_BATTLER_HAS_TRAIT(BATTLE_PARTNER(battlerDef), ABILITY_PASTEL_VEIL))
        return FALSE;
...
bool32 AI_CanBeConfused(u32 battlerAtk, u32 battlerDef, u32 move, u32 ability)
{
    if ((gBattleMons[battlerDef].status2 & STATUS2_CONFUSION)
     || (AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_OWN_TEMPO) && !DoesBattlerIgnoreAbilityChecks(AI_DATA->abilities[battlerAtk], move))
...
{
    if (AI_BATTLER_HAS_TRAIT(battler, ABILITY_MAGMA_ARMOR)
      || AI_BATTLER_HAS_TRAIT(battler, ABILITY_COMATOSE)
      || IS_BATTLER_OF_TYPE(battler, TYPE_ICE)
...
bool32 AnyPartyMemberStatused(u32 battlerId, bool32 checkSoundproof)
{
    ...
    if (IsDoubleBattle())
    {
        ...
        if ((B_HEAL_BELL_SOUNDPROOF == GEN_5 || !AI_BATTLER_HAS_TRAIT(BATTLE_PARTNER(battlerId), ABILITY_SOUNDPROOF) || !checkSoundproof)
...
void IncreasePoisonScore(u32 battlerAtk, u32 battlerDef, u32 move, s32 *score)
{
    if (((AI_THINKING_STRUCT->aiFlags[battlerAtk] & AI_FLAG_TRY_TO_FAINT) && CanAIFaintTarget(battlerAtk, battlerDef, 0))
        || BattlerHeldItemHasEffect(battlerDef, HOLD_EFFECT_CURE_PSN, TRUE) || BattlerHeldItemHasEffect(battlerDef, HOLD_EFFECT_CURE_STATUS, TRUE))
        return;
...
{
    if (((AI_THINKING_STRUCT->aiFlags[battlerAtk] & AI_FLAG_TRY_TO_FAINT) && CanAIFaintTarget(battlerAtk, battlerDef, 0))
            || BattlerHeldItemHasEffect(battlerDef, HOLD_EFFECT_CURE_BRN, TRUE) || BattlerHeldItemHasEffect(battlerDef, HOLD_EFFECT_CURE_STATUS, TRUE))
        return;
