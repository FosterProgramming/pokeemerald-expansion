...
static const u8 sTerrainToType[BATTLE_TERRAIN_COUNT] =
{
    ...
    [BATTLE_TERRAIN_LAB]            = (B_CAMOUFLAGE_TYPES >= GEN_4 ? TYPE_GROUND : TYPE_NORMAL),
};
...
static void Cmd_adjustdamage(void)
{
    ...
    for (battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (BattlerHeldItemHasEffect(gBattlerTarget, HOLD_EFFECT_FOCUS_BAND, TRUE) && rand < param)
        {
        ...
        else if (BattlerHeldItemHasEffect(gBattlerTarget, HOLD_EFFECT_FOCUS_SASH, TRUE) && IsBattlerAtMaxHp(battlerDef))
        {
    ...
    case 4: // lvl up if necessary
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (gBattleResources->bufferB[expBattler][0] == CONTROLLER_TWORETURNVALUES && gBattleResources->bufferB[expBattler][1] == RET_VALUE_LEVELED_UP)
            {
                ...
                u16 gainedSkillPoints = tryToGivePartyMemberExp(&gPlayerParty[*expMonId]);//STRINGID_PKMNGREWTOLV_NO_SKILL
                ...
                PREPARE_BYTE_NUMBER_BUFFER(gBattleTextBuff3, 3, gainedSkillPoints);
                ...
                if(gainedSkillPoints != 0)
                    ...
                    gBattlescriptCurrInstr = BattleScript_LevelUp_NoSkill;
...
bool32 TriggeredSpectralCooconEffect(u8 battler){
    return hasSpectralCocoonEffect(battler) && IsBattleMoveStatus(gCurrentMove);
}
...
static void Cmd_tryconversiontypechange(void)
{
    ...
    if(gBattleMons[gBattlerAttacker].species == SPECIES_PORYGON && VarGet(VAR_BOSS_BRAVE_AI_ID) == BRAVE_BOSS_PORYGON)
        {
...
static void Cmd_tryKO(void)
{
    ...
    else if (BattlerHeldItemHasEffect(gBattlerTarget, HOLD_EFFECT_FOCUS_SASH, TRUE) && IsBattlerAtMaxHp(gBattlerTarget))
    {
