...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, u32 holdEffect)
{
    ...
    {
        if (SearchTraits(battlerTraits, ABILITY_SWIFT_SWIM)       && !BattlerHeldItemHasEffect(battler, HOLD_EFFECT_UTILITY_UMBRELLA, TRUE) && gBattleWeather & B_WEATHER_RAIN)
            speed *= 2;
        else if (SearchTraits(battlerTraits, ABILITY_CHLOROPHYLL) && !BattlerHeldItemHasEffect(battler, HOLD_EFFECT_UTILITY_UMBRELLA, TRUE) && gBattleWeather & B_WEATHER_SUN)
            speed *= 2;
    ...
    if (SearchTraits(battlerTraits, ABILITY_QUICK_FEET) && gBattleMons[battler].status1 & STATUS1_ANY)
        speed = (speed * 150) / 100;
    else if (SearchTraits(battlerTraits, ABILITY_SURGE_SURFER) && gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
        speed *= 2;
    else if (SearchTraits(battlerTraits, ABILITY_SLOW_START) && gDisableStructs[battler].slowStartTimer != 0)
        speed /= 2;
    else if (SearchTraits(battlerTraits, ABILITY_PROTOSYNTHESIS) && !(gBattleMons[battler].status2 & STATUS2_TRANSFORMED) && ((gBattleWeather & B_WEATHER_SUN && WEATHER_HAS_EFFECT) || gBattleStruct->boosterEnergyActivates & (1u << battler)))
        speed = (GetHighestStatId(battler) == STAT_SPEED) ? (speed * 150) / 100 : speed;
    else if (SearchTraits(battlerTraits, ABILITY_QUARK_DRIVE) && !(gBattleMons[battler].status2 & STATUS2_TRANSFORMED) && (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || gBattleStruct->boosterEnergyActivates & (1u << battler)))
        speed = (GetHighestStatId(battler) == STAT_SPEED) ? (speed * 150) / 100 : speed;
...
s8 GetMovePriority(u32 battler, u16 move)
{
    ...
    else if (SearchTraits(battlerTraits, ABILITY_PRANKSTER) && IS_MOVE_STATUS(move))
    {
                              ...
                              u32 holdEffectBattler1, u32 holdEffectBattler2, u32 speedBattler1, u32 speedBattler2, s32 priority1, s32 priority2)
{
    ...
    if (priority1 == priority2)
    {
        ...
        bool32 battler1HasStallingAbility = BattlerHasTrait(battler1, ABILITY_STALL) || (BattlerHasTrait(battler1, ABILITY_MYCELIUM_MIGHT) && IS_MOVE_STATUS(gChosenMoveByBattler[battler1]));
        bool32 battler2HasStallingAbility = BattlerHasTrait(battler2, ABILITY_STALL) || (BattlerHasTrait(battler2, ABILITY_MYCELIUM_MIGHT) && IS_MOVE_STATUS(gChosenMoveByBattler[battler2]));
...
static void TryChangingTurnOrderEffects(u32 battler1, u32 battler2)
{
    ...
    // Battler 1
    // Quick Draw
    if (BattlerHasTrait(battler1, ABILITY_QUICK_DRAW) && !IS_MOVE_STATUS(gChosenMoveByBattler[battler1]) && gBattleStruct->quickDrawRandom[battler1])
    ...
    // Battler 2
    // Quick Draw
    if (BattlerHasTrait(battler2, ABILITY_QUICK_DRAW) && !IS_MOVE_STATUS(gChosenMoveByBattler[battler2]) && gBattleStruct->quickDrawRandom[battler2])
...
bool32 TrySetAteType(u32 move, u32 battlerAtk, u32 attackerAbility)
{
    ...
    if (BattlerHasTrait(battlerAtk, ABILITY_PIXILATE))
        ateType = TYPE_FAIRY;
    if (BattlerHasTrait(battlerAtk, ABILITY_REFRIGERATE))
        ateType = TYPE_ICE;
    if (BattlerHasTrait(battlerAtk, ABILITY_AERILATE))
        ateType = TYPE_FLYING;
    if (BattlerHasTrait(battlerAtk, ABILITY_GALVANIZE))
...
u32 GetDynamicMoveType(struct Pokemon *mon, u32 move, u32 battler, u8 *ateBoost)
{
    ...
    if (gMovesInfo[move].soundMove && BattlerHasTrait(battler, ABILITY_LIQUID_VOICE))
    {
        return TYPE_WATER;
    }
