...
void CustomTrainerPartyAssignMoves(struct Pokemon *mon, const struct TrainerMon *partyEntry)
{
    ...
    for (j = 0; j < MAX_MON_MOVES; ++j)
    {
        ...
        if(GetTrainerClassFromId(TRAINER_BATTLE_PARAM.opponentA) == TRAINER_CLASS_ELITE_FOUR)
    ...
    case BATTLE_INTRO_STATE_DRAW_SPRITES:
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            ...
            else
            {
                ...
                if(battler == 1)
                    VarSet(VAR_ENEMY_1_SPECIES, gBattleMons[battler].species);
                else if(battler == 3)
                    VarSet(VAR_ENEMY_2_SPECIES, gBattleMons[battler].species);
