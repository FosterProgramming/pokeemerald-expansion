...
static u8 ChooseBestMoveAgainstTargetWithLowestHP(u8 battler){
    ...
    if((gBattleMons[sCurrentTarget].species == SPECIES_JOLTEON || gBattleMons[sCurrentTarget].species == SPECIES_FLAREON || gBattleMons[sCurrentTarget].species == SPECIES_EEVEE)
    && VarGet(VAR_BOSS_BRAVE_AI_ID) == BRAVE_BOSS_PORYGON)
        maxScoreMoveId = 1;
